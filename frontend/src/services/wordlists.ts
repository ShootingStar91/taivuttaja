...
const addWord = async (word: string, wordlistId: string, token: string) => {
  try {
    const result = await axios.post<WordList>(url + `/addword/`, { word, wordlistId }, getHeader(token));
    return success<WordList>(result.data);
  } catch (e: any) {
    return error(e);
  }
...
const deleteWord = async (word: string, wordlistId: string, token: string) => {
  try {
    const result = await axios.post<WordList>(url + `/deleteword/`, { word, wordlistId }, getHeader(token));
    return success<WordList>(result.data);
  } catch (e: any) {
    return error(e);
  }
...
const deleteWordlist = async (wordlistId: string, token: string) => {
  try {
    const result = await axios.post<WordList>(url + '/delete/', { wordlistId }, getHeader(token));
    return success<WordList>(result.data);
  } catch (e: any) {
    return error(e);
  }
