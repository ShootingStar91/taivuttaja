...
export const getRandomForm = (
  ...
  const tense =
    selectedTenses[Math.floor(Math.random() * selectedTenses.length)].tense;

  // Filter only those moods that are valid with the selected tense
  const selectedMoods = moodSelections.filter((m) => m.selected);
  const possibleMoods = selectedMoods
    .map((m) => m.mood)
    .filter((m) =>
      validCombinations.find((comb) => comb.mood === m && comb.tense === tense)
    );
