...
export const InfoBar = () => {
  ...
  const goalInfo = () => {
    ...
    if (user.doneWordsToday < user.goal) {
      ...
      <div className='w-full bg-gray-100 rounded-full dark:bg-gray-700'>
        <div className='bg-menu-color text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full' style={{width: percentage}}></div>
      </div>
      </>);
    } else {
  ...
  return (
    <div className='container flex flex-wrap justify-center items-center gap-12 mx-auto p-1'>
      <span className='font-bold'>Logged in as <span className='text-rose-700 font-bold'>{user.username}</span></span>
      <span>{goalInfo()}</span>
    </div>
  );
};
