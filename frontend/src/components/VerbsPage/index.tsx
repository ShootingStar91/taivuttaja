...
export const VerbsPage = () => {
  ...
  const getResults = () => {
    const allResults = allWords.filter(w =>
      w.infinitive.includes(searchText) || w.infinitive_english.includes(searchText));

    const results = allResults.length <= 20 ? allResults : allResults.slice(0, 20);
