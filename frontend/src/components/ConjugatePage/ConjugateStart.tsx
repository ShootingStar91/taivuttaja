...
import {
  ...
  MoodTenseSelection,
  WordListOption,
} from "./ConjugateOptions";

export const ConjugateStart = ({
  startConjugating,
}: {
  startConjugating: (settings: ConjugateSettings) => void;
}) => {
  const initialMoodSelections: MoodSelections = moodList.map((mood) => {
    return { mood, selected: mood === "Indicative" ? true : false };
  });
  const initialTenseSelections: TenseSelections = tenseList.map((tense) => {
    return { tense, selected: tense === "Present" ? true : false };
  });

  const [moodSelections, setMoodSelections] = useState<
    { mood: Mood; selected: boolean }[]
  >(initialMoodSelections);
  const [tenseSelections, setTenseSelections] = useState<
    { tense: Tense; selected: boolean }[]
  >(initialTenseSelections);
    ...
    const getWordLists = async () => {
      ...
      if (
        ...
        result.filter(
          (wl) =>
            wl._id &&
            loadedSettings.wordlist &&
            wl._id === loadedSettings.wordlist._id
        )
      ) {
        if (
          loadedSettings?.wordlist?._id &&
          loadedSettings.wordlist.words.length > 0
        ) {
  ...
  const switchMoodSelection = (mood: Mood) => {
    ...
    const newTenseSelections = tenseSelections.map((s) =>
      validTenses.includes(s.tense) ? s : { tense: s.tense, selected: false }
    );
  ...
  const onStart = async (mode: ConjugateMode) => {
    if (
      !tenseSelections.find((s) => s.selected) ||
      !moodSelections.find((s) => s.selected)
    ) {
          ...
          <>
            {allWordlists && allWordlists.length > 0 ? (
              <WordListOption
                {...{
                  allWordlists,
                  setWordlist: (title: string) => setWordlist(title),
                  wordlistId,
                }}
              />
            ) : (
              <p className="flex justify-center">
                No wordlists found. Create wordlists on user page.
              </p>
            )}
          </>
        )}
          ...
          <div className="container flex flex-wrap justify-center items-center gap-12 mx-auto p-1">
            <p>
              <button
                type="button"
                className="btn"
                onClick={() => onStart(ConjugateMode.Full)}
              >
                All forms
              </button>
            </p>
            <p>
              <button
                type="button"
                className="btn"
                onClick={() => onStart(ConjugateMode.Single)}
              >
                Single
              </button>
            </p>
            <p>
              <button
                type="button"
                className="btn"
                onClick={() => onStart(ConjugateMode.Flashcard)}
              >
                Flashcard
              </button>
            </p>
