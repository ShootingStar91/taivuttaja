...
export const MoodTenseSelection = ({
  availableTenses,
  moodSelections,
  tenseSelections,
  switchMoodSelection,
  ...
  switchMoodSelection: (mood: Mood) => void;
  switchTenseSelection: (tense: Tense) => void;
}) => {
  return (
            ...
            <div key={mood}>
              <label>
                <input
                  type="checkbox"
                  checked={
                    moodSelections.find((m) => m.mood === mood)?.selected
                  }
                  onChange={() => switchMoodSelection(mood)}
                />
                <span className="pl-1">{mood}</span>
              </label>
            </div>
          ))}
        </div>
            ...
            <div key={tense}>
              <label>
                <input
                  type="checkbox"
                  checked={
                    tenseSelections.find((t) => t.tense === tense)?.selected
                  }
                  onChange={() => switchTenseSelection(tense)}
                />
                <span className="pl-1">{tense}</span>
              </label>
            </div>
          ))}
        </div>
...
export const WordListOption = ({
  allWordlists,
  setWordlist,
  wordlistId,
}: {
  allWordlists: WordList[] | null;
  ...
  const getOptions = (): { value: string; label: string }[] => {
    ...
    const list = allWordlists
      ...
      .map((list) => {
        return { label: list.title, value: list._id ? list._id : "" };
      });
...
export const AmountOption = ({
  ...
  onChangeAmount: (e: ChangeEvent<HTMLSelectElement>) => void;
  amount: number;
}) => {
